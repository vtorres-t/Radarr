...
then
    ...
    if [[ -z "$RID" || -z "$FRAMEWORK" ]];
    then
        PackageTests "net9.0" "win-x64"
        PackageTests "net9.0" "win-x86"
        PackageTests "net9.0" "linux-x64"
        PackageTests "net9.0" "linux-musl-x64"
        PackageTests "net9.0" "osx-x64"
        if [ "$ENABLE_EXTRA_PLATFORMS" = "YES" ];
        then
...
then
    ...
    if [[ -z "$RID" || -z "$FRAMEWORK" ]];
    then
        Package "net9.0" "win-x64"
        Package "net9.0" "win-x86"
        Package "net9.0" "linux-x64"
        Package "net9.0" "linux-musl-x64"
        Package "net9.0" "linux-arm64"
        Package "net9.0" "linux-musl-arm64"
        Package "net9.0" "linux-arm"
        Package "net9.0" "linux-musl-arm"
        Package "net9.0" "osx-x64"
        Package "net9.0" "osx-arm64"
        if [ "$ENABLE_EXTRA_PLATFORMS" = "YES" ];
        then
